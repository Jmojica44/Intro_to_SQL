...
SELECT Count(id) AS Count_of_Rows
...
SELECT avg(availability_365) as avg_availability FROM intro_sql.final_airbnb
...
-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood? No - should be one with least availability.

SELECT neighbourhood, Count(neighbourhood) AS count_of_neighbourhood FROM intro_sql.final_airbnb
GROUP BY neighbourhood
ORDER BY count_of_neighbourhood DESC;
...
SELECT host_name, count(id) as number_of_listings FROM intro_sql.final_airbnb
GROUP BY host_name
...
SELECT avg(price) as avg_price FROM intro_sql.final_airbnb;

SELECT id, price, ((price - 165.1027)/165.1027)*100 as percent_from_avg FROM intro_sql.final_airbnb
